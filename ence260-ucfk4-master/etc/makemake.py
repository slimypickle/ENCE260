...
def maincfilename_find(dirname):
    # Only look at existing .c files
    cfiles = glob.glob(os.path.join(dirname, "*.c"))
    if not cfiles:
        return None

    output = subprocess_command('grep -l "main[ ]*(" ' + " ".join(cfiles))
    if not output.strip():
        return None
...
"""
#rmc commented out for Python 2 to 3 conversion
...
def functions_find(filepath, functiondeps, functions, options):
    ...
    for line in text:
        ...
        if matches and matches[0] in functiondeps:
...
def alldeps_print(depsdir, options):
    ...
    for target in list(depsdir.keys()):
