...
void game_render_frame(const game_state_t* s)
{
    ...
    /* Paused overlay (stable): no game elements drawn. */
    if (s->paused) {
        tinygl_draw_char('P', tinygl_point(0, 0));
        return;
    }

    /* Waiting-to-serve overlay (stable). */
    if (s->waiting_to_serve && !s->ball.present) {
    ...
    /* Game elements only: paddle and ball (clean screen). */
